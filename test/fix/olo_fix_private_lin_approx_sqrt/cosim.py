...
from olo_fix import olo_fix_cosim, olo_fix_utils, olo_fix_private_lin_approx_sqrt, olo_fix_plots
from olo_fix.olo_fix_private_lin_approx_sqrt import sqrt_function
...
def cosim(output_path : str = None,
    ...
    # Plot if enabled
    if not cosim_mode:
        expected = sqrt_function(normalized)
        in_data = {"Normalized Value" : normalized}
        out_data = {"sqrt(x)" : result}
    ...
    if cosim_mode:
        ...
        writer.write_cosim_file(normalized, InFmt_g, "Normalized.fix")
