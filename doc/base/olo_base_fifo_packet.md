...
This component implements a synchronous packet FIFO. The FIFO works in store and forward mode i.e. a packet is only presented to the output after it is written into the FIFO completely.

!!! Figure Store and forward with gaps at input !!!
...
* Writing of a packet into a FIFO can be aborted at any time during writing the packet (see [Dropping Packets on Write Side](#Dropping-Packets-on-Write-Side)). The write pointer is automatically rewinded in this case. 
  * Example use-case: A CRC error can only be detected at the end of the packet. User logic can still write the packet into the FIFO directly and just asserts *In_Drop* if a CRC error is detected at the end of the packet.
...
Due to implementation reasons the FIFO introduces one stall cycle per packet on the read-side. Hence the FIFO throughput is suboptimal for very small packets.
...
| MaxPackets_g    | positive  | 17     | Controls how many packets can be stored at maximum in the FIFO (i.e. controlls the size of the packet-size FIFO).<br>Range: 2 ... 2^31-1 |
...
| Out_Size | out     | ceil(log2(*Depth_g*+1)) | N/A | Indicates the size of the currently read packet in words/data-beats. |
...
| Name      | In/Out | Length                  | Default | Description                                                  |
| :-------- | :----- | :---------------------- | ------- | :----------------------------------------------------------- |
| PacketLevel  | out    | ceil(log2(*MaxPackets_g*+1)) | N/A     | Number of packets stored in the FIFO. The counte ris incremented *after* a packet was written completely and decremented *after* the packet was read completely or skipped (due to *Out_Next*). |
...
Example for repeating a packet containin the data 0x1, 0x2, 0x3, 0x4:
...
Remember: Sectoins for next/repeat
