...
| [olo_fix_fir_dec_ser_chpar](./fix/olo_fix_fir_dec_ser_par.md) | Decimating FIR filter (parallel channels, serial tap computation), single- or multi-channel (one multiplier per channel) <br> Runtime-configurable or fixed ratio, tap count and coefficients - Can be used non-decimating (Ratio = 1) |
