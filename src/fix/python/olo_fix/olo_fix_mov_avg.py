...
class olo_fix_mov_avg:
    ...
    def __init__(self,
        ...
        :param gain_corr_coef_fmt: Format of the gain correction coefficient. Must be (0,1,x) - see documentation
        ...
        :param gain_corr_type: Type of gain correction: 
                               ...
                               "SHIFT" = correct gain into the range 0.5 <= gain < 1 by shifting
        ...
        # gain_corr_coef_fmt
        if gain_corr_coef_fmt.S != 0 or gain_corr_coef_fmt.I != 1:
            raise ValueError(f"olo_fix_mov_avg: Invalid gain_corr_coef_fmt: {gain_corr_coef_fmt} (must be (0, 1, x))")
        ...
        self._taps_is_pow2 = np.log2(self._taps).is_integer()
        self.reset()
    ...
    def next(self, in_data):
        ...
        # Gain correction
        if self._gain_corr_type == "NONE":
            return cl_fix_resize(mov_sum, self._mov_sum_fmt, self._out_fmt, self._round, self._saturate)
        elif self._gain_corr_type == "SHIFT" or self._taps_is_pow2:
            return cl_fix_shift(mov_sum, self._mov_sum_fmt, -self._add_bits, self._out_fmt, self._round, self._saturate)
        elif self._gain_corr_type == "EXACT":
            shift_corr = cl_fix_shift(mov_sum, self._mov_sum_fmt, -self._add_bits, self._gain_corr_data_fmt, FixRound.Trunc_s, FixSaturate.Warn_s)
