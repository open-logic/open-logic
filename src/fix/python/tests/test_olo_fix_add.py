...
class TestOloFixAdd(unittest.TestCase):
    ...
    def setUp(self):
        self.add = olo_fix_add(a_fmt=FixFormat(1, 8, 8), b_fmt=FixFormat(1,2,2), result_fmt=FixFormat(1, 8, 8))
        self.a = [-5.5, 3.25, -1.0, 0.0, 4.5]
        self.b = [1.5, -2.25, 0.5, 0.0, -4.5]
        self.expected = [-4.0, 1.0, -0.5, 0.0, 0.0]

    def test_scalar(self):
        self.assertEqual(self.add.process(self.a[0], self.b[0]), self.expected[0])
        self.assertEqual(self.add.process(self.a[1], self.b[1]), self.expected[1])

    def test_array(self):
        result = self.add.process(self.a, self.b)
        self.assertListEqual(list(result), list(self.expected))

    def test_next_scalar(self):
        result = []
        for sa, sb in zip(self.a, self.b):
            result.append(self.add.next(sa, sb))
        self.assertListEqual(result, self.expected)

    def test_next_list(self):
        result = []
        result.extend(self.add.next(self.a[:2], self.b[:2]))
        result.extend(self.add.next(self.a[2:], self.b[2:]))
        self.assertListEqual(result, self.expected)
