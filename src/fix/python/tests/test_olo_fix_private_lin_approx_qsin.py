...
class TestOloFixLinApproxQsin(unittest.TestCase):
    ...
    def setUp(self):
        self.dut = olo_fix_private_lin_approx_qsin(FixFormat(0, -2, 18), FixFormat(1, 0, 16))
    ...
    def test_accuracy_all_configurations(self):
        ...
        for (int_bits, frac_bits), tbl in QSIN_TABLES.items():
            ...
            dut = olo_fix_private_lin_approx_qsin(in_fmt, out_fmt)
    ...
    def test_zero_is_exact(self):
        ...
        for int_bits in [0, 1]:
            dut = olo_fix_private_lin_approx_qsin(FixFormat(0, -2, 16), FixFormat(1, 1, 16))
            self.assertEqual(dut.process(0.0)[0], 0.0)

    def test_peak_scaling(self):
        dut_unscaled = olo_fix_private_lin_approx_qsin(FixFormat(0, -2, 12), FixFormat(1, 1, 16))
        dut_scaled = olo_fix_private_lin_approx_qsin(FixFormat(0, -2, 12), FixFormat(1, 0, 16))
    ...
    def test_unsupported_out_fmt(self):
        ...
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(quarter_fmt(18), FixFormat(0, 0, 16))
        # Fractional bits outside of the supported range
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(quarter_fmt(18), FixFormat(1, 0, 9))
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(quarter_fmt(24), FixFormat(1, 0, 21))
        # Too many integer bits
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(quarter_fmt(18), FixFormat(1, 2, 16))

    def test_unsupported_in_fmt(self):
        # The quarter phase covers one quadrant only, hence it must be (0, -2, N)
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(FixFormat(1, -2, 20), FixFormat(1, 0, 16))
        # Covering a full rotation instead of a quadrant
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(FixFormat(0, 0, 18), FixFormat(1, 0, 16))
        # Covering half a quadrant
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(FixFormat(0, -3, 20), FixFormat(1, 0, 16))
        # Not enough bits to resolve the table index (256 points -> 8 index bits)
        with self.assertRaises(ValueError):
            olo_fix_private_lin_approx_qsin(quarter_fmt(8), FixFormat(1, 0, 16))
