...
class ToolVivado(ToolBase):
    ...
    def get_resource_usage(self) -> dict:
        ...
        for root, _, files in os.walk(self.PROJECT_FOLDER):
            for file in files:
                if file.endswith("utilization_synth.rpt"):
                    summary_file = os.path.join(root, file)
                    break
            if summary_file:
                break
        else:
            raise FileNotFoundError(f"No utilization_synth.rpt file found in the {self.PROJECT_FOLDER} directory or its subdirectories.")
